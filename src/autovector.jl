...
mutable struct AutoVector{T}        #   <: AbstractVector{T}   doesn't work
  ...
  dat::Vector{T}
  function AutoVector{T}(def::T,mini::Integer,
                         maxi::Integer,miniloc::Integer,
...
AutoVector(def::T,
           mini::Integer,
           maxi::Integer,
           miniloc::Integer) where T = 
  AutoVector(def,mini,maxi,miniloc,T[deepcopy(def) for j=1:maxi-mini+1])

"""
AutoVector(x::T)

Create an AutoVector of length 0 with a constant value of x.

For example: v = AutoVector(0.0)
"""
...
Create an AutoVector of length 0 with a constant value of 0.0.

For example: v = AutoVector()
"""
AutoVector() = AutoVector(0.0)
